...
Calls to action now point at the published site or docs for each system, with the exact
artifact the claim was written against kept underneath as a `sourceHref` pin:

| System         | Call to action                                                                                    | Pinned source                                                |
| -------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| Vreko          | `docs.vreko.dev` (`/mcp`, `/how-it-works`, `/mcp-tools`, `/configuration`)                        | `vreko-dev/mcp-server`                                       |
| workspace.json | `workspacejson.dev` (`/spec/`, `/schema/v1.json`, `/getting-started/`, `/implementations/codex/`) | `workspacejson/standard`, `/cli`, `workspace-json/codex-mcp` |
| Interlock      | `interlock.marcellelabs.io` verification cockpit, deep-linked per run                             | `Marcelle-Labs/interlock` at commit `75253e38`               |

Two rows deliberately keep a repository as the call to action, because no published page
covers them: `workspacejson/integrations` (the site's `/conformance/` states the
obligations, not the implementations) and the preregistered `meta-375` study (the Tally
proof ledger is a different artifact and must not stand in for it).

The Interlock deep links use the cockpit's own published contract
(`/media/hac-341/evidence/view-model.json`). The cockpit refuses substitution — an
unrecognised run or state renders "Run unavailable · Substitution refused" rather than
quietly showing the canonical run — so a stale link degrades loudly. `content.test.ts`
holds the run and state ids to that contract.
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Résumé PDF**                        | **Resolved.** A second design import supplied the document; it is ported to `/resume/print` and rendered to `public/` by `pnpm resume:pdf`, one PDF per role lens. | Done. To change the résumé, edit `content/resume.ts` and re-run `pnpm build && pnpm resume:pdf`. ADR 0007.                                                                                                                                                               |
| **LinkedIn URL**                      | **Resolved.** Supplied by the owner: `linkedin.com/in/qmarcelle`.                                                                                                  | Done. Held in `CONTACT` in `content/site.ts`, surfaced as the `linkedin` record in `PROFILES` and in the résumé masthead.                                                                                                                                                |
| **Email address**                     | **Resolved.** Supplied by the owner: `qwynn@marcellelabs.io`. A public address was wanted.                                                                         | Done. Same `CONTACT` record. It is the one evidence destination that is a scheme rather than a page, enumerated out of the absolute-https check in `content.test.ts` by id.                                                                                              |
| **Marcelle Labs URL**                 | **Resolved.** Points at the live site.                                                                                                                             | Done, on the `marcelle-labs` record.                                                                                                                                                                                                                                     |
| **Decision receipt answers** (7)      | **Resolved.** Answered from the Linear issues and Fibery Open Questions that recorded each decision.                                                               | Nothing outstanding. Four receipts carry evidence rows resolving to public revision-pinned artifacts; three name a private record in prose and claim no link. Receipts age with their decisions — re-check any `wouldChangeIf` condition that has since fired. ADR 0006. |
