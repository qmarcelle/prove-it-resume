...
export const linearApplication: ApplicationLens = {
   ...
   *
   * Numbers are absent on purpose. They are stamped from position by `numberSections`,
   * so the rail, the header nav, the skip link and every visible section number are the
   * same sequence by construction. The defect this replaced was the opposite: the proof
   * sections printed the stage they hold on `/` — Vreko `02`, Repository Intelligence
   * `03` — while the rail counted their position here, and three of the sections on the
   * page were in no sequence at all.
   *
  ...
  sections: {
    history: {
      ...
      /*
       * "Nearly a decade" is the durable phrasing in `CAREER`, and it is the phrasing
       * this section keeps. The employment span runs 2016–2026 and the résumé states
       * eight years in technology; rounding that to a flat ten for a reader who would
       * be more impressed by ten is the exact move this site exists to argue against.
       */
      body: 'The AI work below is recent. The habit underneath it is not: nearly a decade inside regulated healthcare engineering, where a release that looks fine and is wrong costs more than one that fails loudly. Customer-facing platform initiatives — Portal Refresh, CIAM, Shared Health — alongside the microservice architecture, identity, and delivery pipelines that carry them, then two and a half years leading the teams doing both halves.',
      boundary:
        'Themes are surfaced in place of employer-confidential details, and the named initiatives are enterprise programmes led or aligned across teams rather than products owned end to end by one engineer. The conventional chronology, with titles and dates, is in the résumé below.',
    },
    inPractice: {
